...
def run_smoothgrad(
    ...
    if image.shape[2:] != (96, 96, 96):
        image = F.interpolate(image, size=(96, 96, 96), mode="trilinear", align_corners=False)
    # SmoothGrad
    smooth_grad = SmoothGrad(model, stdev_spread=0.15, n_samples=1, magnitude=True, verbose=True)
    ...
    plt.tight_layout()
    os.makedirs(output_dir, exist_ok=True)
    save_path = os.path.join(output_dir, f"smoothgrad_sample{sample_idx}_class{target_class}_channel{channel_idx}.png")
    plt.savefig(save_path)
    print(f"[INFO] Saved SmoothGrad overlay to {save_path}")
    plt.close()
    return input_np, attributions
...
def main():
    ...
    parser.add_argument("--method", type=str, default="gradcam", choices=["gradcam", "saliency", "smoothgrad"], help="Visualization method: gradcam, saliency, or smoothgrad")
